...
2. Background poll every N minutes (configurable, default: 60):
   GET /api/stats → compare pending.total against last known count

3. If pending.total increased → fire browser notification:
   "2 new decisions pending review in Quorum — auth:token-strategy, db:migrations"

4. Badge on Pending tab in sidebar updates in real-time
...
│  GET  /.well-known/jwks     GET  /api/graph          │
│  POST /graphiti/*           GET  /api/knowledge      │
│  GET  /pg/versions/*        POST /api/review/:id     │
│  GET  /pg/pending           GET  /api/search         │
│  PATCH /pg/pending/:id                               │
│  GET  /pg/audit                                      │
...
| UI components | shadcn/ui + Tailwind | Consistent, unstyled-first, no vendor lock-in |
...
3. Frontend: POST /auth/token { github_token, project_id }
4. Gateway: verifies token with GitHub API → signs ES256 JWT
5. Frontend: stores JWT in React context (memory only)
6. All subsequent API calls: Authorization: Bearer <JWT>
7. JWT expiry (1 hour): re-auth prompt appears
```

JWT claims available to the frontend (decoded client-side):
- `sub` — GitHub username (display name, identity)
- `project` — active project namespace
- `role` — engineer | tech_lead | architect | principal_architect
- `team` — team name (for filtering / display)
- `base_confidence` — default confidence for this member's writes
...
- On save: uploads to S3, calls `POST /config/:projectId/invalidate` to clear cache

**Why this is important:** Role and seniority signals for the authority model become configurable here — no code change needed to update the org hierarchy.
...
Five new routes added to the existing Express app (`src/gateway/routes/dashboard.js` — new file).

All routes require JWT. All are project-scoped.
...
3. Gateway BFF routes — `src/gateway/routes/dashboard.js` (6 new routes including `/api/bump`)
...
2. **Multi-project UI:** The JWT carries one project at a time. Should the dashboard support switching projects without re-authenticating? Requires a project-picker on the login screen or a re-auth flow per project switch.
...
4. **Config write path:** The Gateway currently reads config from S3 but has no S3 write route. The config editor needs a `PUT /config/:projectId` endpoint added to the Gateway that uploads to S3 then invalidates cache. This is a new Gateway route not currently in the codebase.
5. **Notification poll granularity:** The poll fires `GET /api/stats` and checks `pending.total`. This only tells reviewers *how many* new decisions exist, not *which ones*. Consider storing the last-seen decision IDs client-side and diffing to show specific topic:key pairs in the browser notification.
