...
**P4 gaps (in progress):** GAP-021 ✅ — Admin filtered audit log: `writeGovernanceAudit` in `gateway/src/shared/audit/governance.js` now resolves `project` (group_id string) → `q_project_id` (numeric DB ID) via `getProjectByGroupId` before calling `writeAuditEntry`, so `role_update` entries are project-scoped and findable via `GET /pg/audit?tool=role_update`; `admin_add`/`admin_remove` keep `project: null` intentionally (platform-level, cross-project). S-09.7 (3 E2E tests) added to `09-admin-operations.spec.js`. Gateway unit tests: 697 passed (+9 from P3 GIT tests). E2E suite: 541 passed, 1 skipped.
