...
**Hierarchy-anchor DDB pollution fix (2026-07-06):** Local dev surfaced org-hierarchy scaffolding nodes (`ayan-org`, `group-a/b/c`, `div-a/b/c`, `dep-a/b/c`) in a user's `GET /user/profile/:username` project list and dashboard project switcher — every hierarchy anchor's config carries a `members` entry (needed so `owner`/`principal_architect` validation passes), and `POST /config/upload` → `syncOneProject()` synced that member into DynamoDB exactly like a real project. Fix: `is_hierarchy_anchor: z.boolean().default(false)` added to `QuorumConfigSchema` (`quorum-mcp/src/config/schema.js` canonical + `gateway/src/shared/config/schema.js` vendored copy, synced same commit); `syncOneProject()` in `gateway/src/routes/sync.js` now skips the `syncProjectMembers()` DDB write when `config.is_hierarchy_anchor === true`. Anchor nodes still resolve correctly for `/api/portfolio`, which walks `hierarchy.parent` directly against Postgres/S3 and never touches DDB. `scripts/seed-global-catalogs.js`'s `nodeConfig()` updated to set the flag on all 10 anchor configs it generates; existing local S3 objects and DDB rows patched by hand to reflect the fix immediately rather than waiting for a full `docker:clean:all` re-seed. See [ARCHITECTURE.md](docs/ARCHITECTURE.md#organisational-hierarchy). **Follow-up (open, unconfirmed):** the gateway's own boot-time `syncAllConfigs()` (`server.js` `startup()`) is a separate S3→DDB sync path from `init-localstack.sh` and races it on `docker:start` — if the gateway container's automatic sync runs before `init-localstack.sh` re-uploads corrected local config files to the external, persistent LocalStack S3 bucket, stale pre-fix S3 objects can repopulate DDB. Currently dormant (S3 holds corrected data), worked around by a manual `docker restart quorum-gateway-1` when it resurfaces; not yet structurally hardened (e.g. reordering `setup.sh` or triggering a resync after `init-localstack.sh` finishes).

**Global catalog expansion — best-practice standards (2026-07-09):** `scripts/seed-global-catalogs.js` extended from 5 to 12 global catalogs (120 total ACTIVE entries, 10 per catalog, seeded via admin JWT against `POST /pg/versions`): `architecture-principles` (SOA, KISS, DRY, SLAP, SRP, YAGNI, Law of Demeter, composition-over-inheritance, dependency inversion, fail-fast), `owasp-standards` (OWASP Top 10 2021, one entry per A01–A10), `performance-standards` (N+1 prevention, caching, indexing, algorithmic complexity, async I/O, load testing, bundle budget, CDN, lazy loading, memory leaks), `testing-standards` (test pyramid, no live network in unit tests, flaky-test quarantine, contract testing, regression-test-on-bugfix, test data isolation, CI parallelization, mocking boundary, mutation testing, behavior-driven naming), `observability-standards` (correlation IDs, distributed tracing, RED/USE metrics, SLOs, symptom-based alerting, dashboards, log retention, runbooks, blameless postmortems, synthetic monitoring), `documentation-standards` (ADRs, README minimums, OpenAPI, why-not-what comments, changelogs, diagrams-as-code, doc freshness, deprecation notices, onboarding docs, on-call runbooks), and `ai-systems-standards` (MCP least privilege, agent destructive-action confirmation, prompt-injection defense, RAG grounding/citation, LLM output schema validation, human-in-the-loop for ambiguity, single-purpose skill scoping, model selection cost/latency fit, eval suite before prompt changes, AI action audit logging). New catalogs parented under `dep-a`/`dep-b`/`dep-c` alongside the original 5 for balanced hierarchy distribution.

**Global catalogs made `is_public: true` (2026-07-09):** all 12 catalog configs now carry `is_public: true` so any authenticated user can read them without being a project member — enforced in `gateway/src/middleware/verify-jwt.js` (`accessDenied = !(config?.is_public === true)` for non-members). This is post-login read access only; there is no unauthenticated/no-JWT path (`verifyJwt` returns `401 missing_token` with no Bearer token). Applying the field to already-onboarded projects required a new fallback in `scripts/seed-global-catalogs.js`'s `uploadConfig()`: `POST /config/upload` is create-only and returns `409 already_onboarded` for existing projects (per the non-negotiable rules table above), so `uploadConfig()` now catches that 409 and retries via `PUT /config/:groupId` (added `put()` helper alongside `post()`). Re-run with `QUORUM_CONFIG_ONLY=1` to push config-only changes like this without re-inserting duplicate ACTIVE knowledge versions. Documented as a standard fresh-deploy bootstrap step (alongside `seed:admin`) in [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md#seeding-global-catalogs-recommended-for-a-fresh-deploy) — a real-AWS deploy is not seeded automatically and this must be run explicitly against the target gateway.

**Graphiti session/connection stability fixes (2026-07-14):** Live `search()` verification against the `busy-hopper` onboarding project surfaced two distinct FalkorDB/Graphiti reliability bugs, both now fixed.
- **Connection-pool exhaustion:** `redis-py`'s async `ConnectionPool` hardcodes `max_connections=100`. A prior request storm left exactly 100 connections permanently stuck `in_use` inside `quorum-graphiti-1` (confirmed via `CLIENT LIST`, ~10h old — a genuine leak, not a fresh burst), causing every subsequent Graphiti call to fail with `MaxConnectionsError`. `SEARCH_CONCURRENCY=4` (a client-side throttle already in `quorum-mcp/src/tools/search.js`, documented in `quorum-mcp/CLAUDE.md`) prevents *new* storms but cannot free connections already wedged in another process's memory. Fix: `docker restart quorum-graphiti-1` (resets in-memory pool state) — no source change needed for this half. The real fix (caching per-driver "indices built" state to avoid `FalkorDriver.clone()` re-running `build_indices_and_constraints()` on every call) belongs in the `quorum-graphiti` fork per the workspace root `CLAUDE.md`'s Graphiti fork rules.
- **Stale MCP session never invalidated on 404:** `callGraphiti()` in both `gateway/src/shared/graph/client.js` and the canonical `quorum-mcp/src/graph/client.js` caches Graphiti's raw MCP `Mcp-Session-Id` in a module-level `_sessionId` singleton. The reset/retry guard only matched HTTP 400, but Graphiti's actual "Session not found" error (e.g. after a Graphiti container restart drops its in-memory session store while the gateway process itself keeps running) comes back as **404** — so a dead session was resent forever until the gateway process was restarted. Fixed by widening both guards (the `!response.ok` reset and the `catch` block's retry-continue) to also match 404, in both files. See `gateway/CLAUDE.md`'s Shared Modules section and `quorum-mcp/CLAUDE.md`'s Post-Wave-G additions for full detail.
- Verified live via two consecutive `mcp__quorum__search()` calls against `busy-hopper` post-fix: zero `MaxConnectionsError`, zero "Session not found" in either debug log (`~/.quorum/calls/*.log`). Remaining timeout errors observed in those logs are the separate, already-documented `graphiti-core` index-rebuild inefficiency above, not a regression from either fix; `search()` still completes via the Postgres ILIKE fallback in both cases.
