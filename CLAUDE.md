...
- Shared physical graph database for global catalogs: `migrated_to_shared_graph: boolean` (optional, `QuorumConfigSchema`) — only meaningful when `is_global: true`. A global catalog config carrying `migrated_to_shared_graph: true` has its writes routed to one shared physical FalkorDB database (`QUORUM_SHARED_GRAPH_DATABASE`, default `quorum_shared_globals`) instead of the database implied by its own `group_id`. `group_id` (logical isolation) is unaffected — reads still scope by `group_ids` as before. Injected server-side in `gateway/src/routes/graphiti.js` for write ops only (never on `search_nodes`/`search_memory_facts`), at both the legacy `body.params` level and the MCP-protocol `body.params.arguments` level, mirroring the existing `group_id`/`group_ids` injection pattern. The underlying `database` override on `addEpisode`/`addSupersedingEpisode` (`gateway/src/shared/graph/client.js`, vendored in `quorum-mcp/src/graph/client.js`) traces back to `graphiti_core.Graphiti.add_episode`'s `database` parameter (quorum-graphiti fork). One-time backfill for catalogs flipped to `migrated_to_shared_graph: true` after they already have ACTIVE knowledge: `node scripts/reconcile-globals.js [--dry-run] [--project <group_id>]` (`npm run job:reconcile-globals`) — discovers candidates via `q_projects.is_global = TRUE` (PostgreSQL) enriched by `loadProjectConfig()` (S3/Redis, the only place `migrated_to_shared_graph` itself lives), then re-ingests each catalog's `ACTIVE` `knowledge_versions` rows into the shared database.
