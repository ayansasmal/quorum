...
async function remember(qProjectId, params) {
  ...
  try {
    ...
    const status = active
      ? `v${active.version} → v${nextVersion}`
      : `v${nextVersion} (new)`;
...
async function seed() {
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'token-strategy',
    content:
      'Use session tokens for all services. Session tokens allow revocation and work well for our monolithic application architecture.',
    author: 'junior-dev',
    confidence: 0.6,
    entity_type: 'Decision',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'token-strategy',
    content:
      'Use JWT for all services. JWTs are stateless, scalable, and work across all deployment types including serverless.',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'token-strategy',
    content:
      'Use JWT for Lambda-based services (payment-svc, notification-svc). Use session tokens for ECS-based internal services (auth-svc, admin-svc) where revocation is needed. The split follows deployment type, not service domain.',
    author: 'ayan',
    confidence: 0.9,
    reason:
      'ADR-042 nuanced after Lambda constraint discovered in payment-svc — blanket JWT was too broad',
    entity_type: 'Decision',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'delegation-flow',
    content:
      'Internal service-to-service auth uses mTLS with certificates issued by our internal CA. External-facing APIs use OAuth 2.0 client credentials. Never use API keys for service-to-service — they cannot be rotated without downtime.',
    author: 'senior-architect',
    confidence: 0.9,
    entity_type: 'Decision',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'rate-limiting',
    content:
      'Rate limiting is applied at the API gateway level, not per service. Limits: 100 req/s per authenticated user, 10 req/s for unauthenticated. Use sliding window algorithm. Exceeded limits return 429 with Retry-After header.',
    author: 'engineer',
    confidence: 0.8,
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'error-standards',
    content:
      'All API errors return JSON with shape: { error: { code: string, message: string, request_id: string } }. HTTP status codes follow RFC 7807. Never expose internal stack traces. Log errors server-side with request_id for correlation.',
    author: 'senior-architect',
    confidence: 0.95,
    entity_type: 'Pattern',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'versioning',
    content:
      'API versioning uses URL path prefix: /v1/, /v2/. Minor version changes (new optional fields) are backwards compatible and do not increment major version. Deprecation notice period is minimum 6 months. Breaking changes require new major version.',
    author: 'senior-architect',
    confidence: 0.9,
    entity_type: 'Decision',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'pagination',
    content:
      'Use cursor-based pagination for all list endpoints. Response includes: { data: [], next_cursor: string | null, has_more: boolean }. Cursor is opaque — clients must not parse it. Max page size is 100. Default page size is 20.',
    author: 'engineer',
    confidence: 0.85,
    entity_type: 'Pattern',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'PostgreSQL connection pool size: 10 connections per service instance. Pool per service, not per request. Use pg.Pool with idleTimeoutMillis: 30000. Never exceed 100 total connections across all instances — database max_connections is 200 with 100 reserved for admin.',
    author: 'senior-architect',
    confidence: 0.85,
    entity_type: 'Constraint',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'Database migrations run automatically on service startup using node-pg-migrate. Migrations are append-only in development. Production migrations require: (1) backwards-compatible schema change, (2) code deploy, (3) cleanup migration if needed. Never DROP COLUMN in same release as code change.',
    author: 'senior-architect',
    confidence: 0.9,
    entity_type: 'Runbook',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'Table names: snake_case plural (users, api_keys, audit_logs). Column names: snake_case. Primary keys: id (serial or uuid). Foreign keys: <table_singular>_id. Timestamps: created_at, updated_at (timestamptz). Boolean columns: is_ or has_ prefix.',
    author: 'engineer',
    confidence: 0.8,
    entity_type: 'Pattern',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'Secrets are stored in AWS Secrets Manager. Never in environment variables committed to git. Never in .env files in production. Services retrieve secrets on startup via SDK — not via env injection. Secret rotation is automated via Lambda rotators. Rotation period: 90 days for API keys, 30 days for DB passwords.',
    author: 'senior-architect',
    confidence: 0.95,
    entity_type: 'Constraint',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'retry-strategy',
    content:
      'Use exponential backoff with jitter for all external service calls. Base delay: 1s. Max delay: 30s. Max retries: 3. Jitter: ±20%. Do not retry on 4xx (client errors). Always retry on 5xx and connection timeouts. Implement circuit breaker at 50% failure rate over 1 minute window.',
    author: 'engineer',
    confidence: 0.8,
    entity_type: 'Pattern',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'unit-strategy',
    content:
      'Unit test pure business logic — calculations, transformations, validations. Do not unit test framework wiring, trivial getters/setters, or code that only makes sense as integration. Coverage target: 80% on business logic files. Use Vitest. Mock at service boundaries, not inside services.',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'Integration tests run against real PostgreSQL and real Redis (via docker-compose in CI). Never mock the database — we burned a release when mocked tests passed but prod migration failed. API tests use real HTTP client against running server. External services (Stripe, AWS) are stubbed at the HTTP level only.',
    author: 'senior-engineer',
    confidence: 0.9,
    entity_type: 'Constraint',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'Guest checkout must remain available. Conversion research shows 40% of users abandon when forced to create an account before purchasing. Any removal or restriction of guest checkout requires product owner sign-off and a fresh conversion analysis before shipping.',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    content:
      'All EU user data must be stored and processed exclusively in eu-west-1. Required for GDPR compliance with enterprise customers. Applies to: user profiles, order history, payment records. No cross-region replication of EU data to non-EU regions without explicit DPA amendment.',
    author: 'legal-team',
    confidence: 0.95,
    entity_type: 'Requirement',
  ...
  await remember(Q_PROJECT_ID, {
    ...
    key: 'connection-pooling',
    content:
      'For high-concurrency batch processing services, use connection pool size of 50. Standard pool size of 10 is a bottleneck under load.',
    author: 'junior-dev',
    confidence: 0.5,
    entity_type: 'Constraint',
  ...
  console.log(
    '  node scripts/audit-cli.js lineage compliance gdpr-data-residency\n',
  );
}
