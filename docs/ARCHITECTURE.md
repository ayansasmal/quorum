...
- **Dashboard write (PE only):** `POST /api/knowledge` / `promote` / `supersede` → role gate → `validateKnowledgeInput` → PostgreSQL (`insertVersion` or `atomicSupersede`) → Graphiti (via `/graphiti/*`) → `writeAuditEntry`. `author_type: 'human'`, `triggered_by: 'dashboard'`. Rate-limited 10/min/IP, 4 KB payload cap.
