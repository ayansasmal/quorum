...
| `tests/gateway/auth.test.js` (additions) | engram | `POST /auth/token` accepts an optional `project_id`, never gates issuance on membership or config lookup, and keeps JWT claims limited to `{ sub, is_admin, jti, exp, iat }`; `GET /auth/projects` → 410; `POST /auth/switch` → 410 |
| `tests/gateway/ensure-admin-config.test.js` | engram | Gateway boot seed no-op, comma-separated admin parsing, S3 `IfNoneMatch: '*'` atomicity, existing-config idempotency, and concurrent-writer `412` handling |
| `tests/gateway/require-membership.test.js` | engram | Public-project reads remain available to authenticated non-members while POST/PATCH/PUT/DELETE require a project role or platform-admin status |
| `tests/gateway/admin.test.js` (G3 additions) | engram | Removing the final platform admin returns `409 last_admin`, both for self-demotion and for another still-authenticated admin |
