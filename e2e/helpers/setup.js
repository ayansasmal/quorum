...
/**
 * Polls a service URL until it becomes reachable or times out.
 *
 * @param {string} url - Absolute URL to probe.
 * @param {string} label - Human-readable service name for error messages.
 * @param {number} [timeoutMs=60_000] - Max time to wait before failing.
...
async function waitFor(url, label, timeoutMs = 60_000) {
  ...
  throw new Error(`T0 FAIL — ${label} not reachable after ${timeoutMs / 1000}s. Last error: ${last}`)
}
...
export default async function setup(_config) {
  ...
  if (DASHBOARD_URL && !DASHBOARD_URL.startsWith('http://localhost')) {
    await waitFor(DASHBOARD_URL, 'dashboard')
