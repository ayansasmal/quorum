# Quorum
## Persistent Engineering and Business Memory for Codex and AI Agents

Quorum is an open-source **governance layer** for engineering and business knowledge built on Graphiti's
temporal knowledge graph. It gives Codex and multi-agent systems a shared,
...
```mermaid
graph TD
    CC[Codex / AI Agents] -->|MCP stdio| MCP[Quorum MCP Server\nNode.js :8000]
...
**Key flows:**
- Engineers connect the MCP server via `Codex mcp add quorum`; it always talks to the gateway over HTTP (default: `http://localhost:3001`)
...
**AWS deployment TLS decision (2026-06-11):**
- The Crossplane AWS design requires a real domain whose A record points directly to the EC2 Elastic IP.
- Caddy terminates TLS on the instance and automatically obtains/renews a publicly trusted ACME certificate; GitHub OAuth and gateway URLs use the same domain.
...
- Bootstrap downloads the AWS global RDS CA bundle, mounts it read-only into every gateway-image service, and sets `NODE_EXTRA_CA_CERTS`; do not disable certificate verification to make RDS TLS work.
...
| Codex writes always DRAFT | `author === 'Codex'` forces DRAFT in `storeFirst()` — in `quorum-mcp` |
