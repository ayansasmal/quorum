...
export default function VersionTimeline({ versions = [] }) {
  ...
  return (
    <ol className="relative ml-2 border-l border-gray-700 space-y-4 py-1">
      {versions.map((v) => (
        <li key={v.version} className="ml-4">
          <span
            className={`absolute -left-1.5 mt-0.5 h-3 w-3 rounded-full border-2 border-gray-900 ${
          ...
          <div className="space-y-0.5">
            <div className="flex items-baseline gap-2">
              <span className="text-xs font-mono font-bold text-gray-300">v{v.version}</span>
            ...
            <p className="text-xs text-gray-500">
              by <span className="text-gray-400">{v.author}</span>
