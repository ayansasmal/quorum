...
export default function NodePanel({ node, onClose }) {
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-72 bg-white/95 dark:bg-gray-900/95 border-l border-gray-200 dark:border-gray-800 overflow-y-auto shadow-2xl z-10">
      ...
      <div className="p-4 space-y-3">
        ...
        {d.summary && <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{d.summary}</p>}
        <div className="text-xs text-gray-600 space-y-1">
          <p>Author: <span className="text-gray-500 dark:text-gray-400">{d.author ?? '—'}</span></p>
          <p>Status: <span className="text-gray-500 dark:text-gray-400">{d.status}</span></p>
        </div>
      </div>
    </div>
  )
